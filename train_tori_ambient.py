...
tori_dim = next(iter(trainloader)).shape[1]
print(tori_dim)
...
theta = transform(next(iter(trainloader)))
...
if args.imp and args.mode=='train':
    ...
    for _ in range(100):
        x = tori_theta_to_ambient(transform(iter(trainloader).next()))
        if cuda:
            x = x.cuda()
...
if os.path.exists(os.path.join(folder_path, 'checkpoint.pt')):
    ...
    a.eval()
    tori_dim = next(iter(trainloader)).shape[1]  # dataset is expressed in theta(intrisic coordinates)
    wandb.log(
        {'samples': wandb.Image(
            plotting.fig2pil(
...
def evaluate_step():
    train_exact_logp = evaluate_exact_logp(dataloader=trainloader, a_func=a, bm_sde=sde, prior=prior)
